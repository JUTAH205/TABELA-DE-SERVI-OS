...
// Which form types need which tabs
const TABS_CONFIG = {
...
const TAB_LABELS = {
  ...
  embarcacao: "Embarcacao/Motor",
  documentos: "Documentos",
  palamenta: "Palamenta/Seguranca",
  atividade: "Atividade/Capturas",
...
const defaultForm = () => ({
  tipo_atividade: "", data: new Date().toISOString().split("T")[0], hora: new Date().toTimeString().slice(0,5), gps_n: "", gps_w: "", local: "", status: "legal",
  timoneiro_nome: "", timoneiro_estado_civil: "", timoneiro_morada: "", timoneiro_cc: "", timoneiro_cc_validade: "", timoneiro_data_nasc: "", timoneiro_contato: "", timoneiro_naturalidade: "", timoneiro_profissao: "", timoneiro_nif: "", timoneiro_filiacao: "", timoneiro_carta: "", timoneiro_im: "", timoneiro_categoria: "", timoneiro_carta_validade: "",
  mesmo_proprietario: false,
  proprietario_nome: "", proprietario_nif: "", proprietario_morada: "", proprietario_cc: "", proprietario_cc_validade: "", proprietario_data_nasc: "", proprietario_contato: "",
  ...
  operador_nome: "", operador_nif: "", operador_morada: "", operador_cae: "", operador_contato: "", operador_licenca_num: "", operador_licenca_validade: "", embarcacoes_associadas: [],
  observacoes: "",
});
...
export default function FiscalizacaoNovo() {
  ...
  useEffect(() => {
    if (isEditing) {
      setLoading(true);
      api.getFisc(id).then(data => {
        setForm(data);
        setSelectedTipo(data.tipo_atividade);
        setStep("form");
        setLoading(false);
    ...
    if (step === "form" && !isEditing && navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          ...
          const lonM = ((Math.abs(lon) - lonD) * 60).toFixed(3);
  ...
  const selectTipo = (t) => {
    setSelectedTipo(t);
    setForm({ ...defaultForm(), tipo_atividade: t });
    setStep("form");
    ...
    return (
      <div data-testid="fisc-select-page" className="max-w-4xl mx-auto space-y-6">
        <h1 className="text-2xl font-bold tracking-tight uppercase" style={{ fontFamily: "Barlow, sans-serif", color: "#002D72" }}>
          Nova Fiscalizacao - Selecionar Tipo
        </h1>
    ...
    <div data-testid="fisc-form-page" className="max-w-5xl mx-auto space-y-4">
      ...
      <div className="sticky top-0 z-20 bg-white border border-slate-200 rounded-sm p-3 shadow-sm" data-testid="fisc-sticky-header">
        <div className="flex flex-wrap items-center gap-4 text-xs">
          <div className="font-mono font-bold" style={{ color: "#002D72" }}>
            {isEditing ? form.numero_fiscalizacao : (nextNum || "...")}
          </div>
          <div className="flex items-center gap-2">
            <Label className="text-xs text-muted-foreground">Data:</Label>
            <Input type="date" value={form.data} onChange={e => u("data", e.target.value)} className="rounded-sm h-7 text-xs w-36" />
          </div>
          <div className="flex items-center gap-2">
            <Label className="text-xs text-muted-foreground">Hora:</Label>
            <TimeInput value={form.hora} onChange={e => u("hora", e.target.value)} className="rounded-sm h-7 text-xs w-20" />
          </div>
          <div className="flex items-center gap-1">
            <MapPin className="h-3.5 w-3.5 text-muted-foreground" />
            <Label className="text-xs text-muted-foreground">N:</Label>
            <Input value={form.gps_n} onChange={e => u("gps_n", e.target.value)} className="rounded-sm h-7 text-xs w-24" placeholder="00 00.000" />
            <Label className="text-xs text-muted-foreground">W:</Label>
            <Input value={form.gps_w} onChange={e => u("gps_w", e.target.value)} className="rounded-sm h-7 text-xs w-24" placeholder="00 00.000" />
          </div>
          <div className="flex items-center gap-2">
            <Label className="text-xs text-muted-foreground">Local:</Label>
            <UpperInput value={form.local} onChange={e => u("local", e.target.value)} className="rounded-sm h-7 text-xs w-32" />
          </div>
          <Select value={form.status} onValueChange={v => u("status", v)}>
            <SelectTrigger className={`h-7 text-xs w-28 rounded-sm font-bold ${form.status === "legal" ? "bg-green-50 text-green-700 border-green-300" : "bg-red-50 text-red-700 border-red-300"}`} data-testid="campo-status">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
      ...
      <div className="form-section min-h-[300px]">
          ...
          <div className="space-y-5">
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                <div><Label className="text-xs text-muted-foreground">NIF</Label><Input value={form.timoneiro_nif} onChange={e => u("timoneiro_nif", e.target.value)} className="rounded-sm mt-1" /></div>
                ...
                <div><Label className="text-xs text-muted-foreground">N. CC</Label><Input value={form.timoneiro_cc} onChange={e => u("timoneiro_cc", e.target.value)} className="rounded-sm mt-1" /></div>
                <div><Label className="text-xs text-muted-foreground">Validade CC</Label><DateInput value={form.timoneiro_cc_validade} onChange={e => u("timoneiro_cc_validade", e.target.value)} className="rounded-sm mt-1" /></div>
                ...
                <div><Label className="text-xs text-muted-foreground">Validade</Label><DateInput value={form.timoneiro_carta_validade} onChange={e => u("timoneiro_carta_validade", e.target.value)} className="rounded-sm mt-1" /></div>
              </div>
            </div>
            <div className="flex items-center gap-2 p-2 bg-slate-50 rounded-sm">
              ...
              <Label className="text-xs font-semibold">Proprietario e o mesmo que Timoneiro</Label>
            </div>
            {!form.mesmo_proprietario && (
              <div>
                <div className="form-section-title">Proprietario / Operador</div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  <div className="sm:col-span-2"><Label className="text-xs text-muted-foreground">Nome</Label><UpperInput value={form.proprietario_nome} onChange={e => u("proprietario_nome", e.target.value)} className="rounded-sm mt-1" /></div>
                  <div><Label className="text-xs text-muted-foreground">NIF</Label><Input value={form.proprietario_nif} onChange={e => u("proprietario_nif", e.target.value)} className="rounded-sm mt-1" /></div>
                  <div className="sm:col-span-2"><Label className="text-xs text-muted-foreground">Morada</Label><UpperInput value={form.proprietario_morada} onChange={e => u("proprietario_morada", e.target.value)} className="rounded-sm mt-1" /></div>
                  <div><Label className="text-xs text-muted-foreground">N. CC</Label><Input value={form.proprietario_cc} onChange={e => u("proprietario_cc", e.target.value)} className="rounded-sm mt-1" /></div>
                  <div><Label className="text-xs text-muted-foreground">Validade CC</Label><DateInput value={form.proprietario_cc_validade} onChange={e => u("proprietario_cc_validade", e.target.value)} className="rounded-sm mt-1" /></div>
                  <div><Label className="text-xs text-muted-foreground">Data Nasc.</Label><DateInput value={form.proprietario_data_nasc} onChange={e => u("proprietario_data_nasc", e.target.value)} className="rounded-sm mt-1" /></div>
                  <div><Label className="text-xs text-muted-foreground">Contato</Label><Input value={form.proprietario_contato} onChange={e => u("proprietario_contato", e.target.value)} className="rounded-sm mt-1" /></div>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="space-y-5">
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div><Label className="text-xs text-muted-foreground">Nome</Label><UpperInput value={form.embarcacao_nome} onChange={e => u("embarcacao_nome", e.target.value)} className="rounded-sm mt-1" data-testid="campo-embarcacao-nome" /></div>
                <div><Label className="text-xs text-muted-foreground">CON.ID.</Label><UpperInput value={form.embarcacao_con_id} onChange={e => u("embarcacao_con_id", e.target.value)} className="rounded-sm mt-1" /></div>
              ...
              {DOC_FIELDS.map(f => (
                <div key={f} className="flex items-center gap-2 p-2 bg-slate-50 rounded-sm border border-slate-100">
                  ...
                  <div className="flex-1">
                    ...
                    <DateInput value={form.documentos[f]?.validade || ""} onChange={e => uDoc(f, "validade", e.target.value)} className="rounded-sm mt-1 h-7 text-xs" placeholder="DD/MM/AAAA" />
                  </div>
                </div>
              ))}
            </div>
        ...
        {activeTab === "palamenta" && (
          <div>
            <div className="form-section-title">Meios de Salvacao e Equipamentos</div>
              ...
              <div className="space-y-2 mt-2">
                {(form.especies || []).map((esp, i) => (
                  <div key={i} className="flex gap-2 items-center">
                    ...
                    <Input value={esp.peso_kg} onChange={e => { const s = [...form.especies]; s[i] = { ...s[i], peso_kg: e.target.value }; u("especies", s); }} className="rounded-sm text-xs w-24" placeholder="PESO KG" type="number" />
                    {form.especies.length > 1 && <button onClick={() => { const s = form.especies.filter((_, j) => j !== i); u("especies", s); }} className="p-1 hover:bg-red-50 rounded-sm"><Trash2 className="h-3 w-3 text-red-500" /></button>}
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-2 mt-2">
                {(form.artes || []).map((art, i) => (
                  <div key={i} className="flex gap-2 items-center">
                    ...
                    {form.artes.length > 1 && <button onClick={() => { const s = form.artes.filter((_, j) => j !== i); u("artes", s); }} className="p-1 hover:bg-red-50 rounded-sm"><Trash2 className="h-3 w-3 text-red-500" /></button>}
                  </div>
                ))}
              </div>
            </div>
          ...
          <div>
            ...
            <div className="overflow-x-auto">
              <table className="service-table w-full text-xs">
                <thead>
                  <tr><th>Equipamento</th><th>Zona 1</th><th>Zona 2</th><th>Zona 3</th><th>Zona 4</th><th>Zona 5</th></tr>
                </thead>
                <tbody>
                  {[
                    ["Coletes (1 por pessoa)", "Sim", "Sim", "Sim", "Sim", "Sim"],
                    ["Boia circular c/ retenida", "1", "1", "1", "-", "-"],
                    ["Extintor", "1", "1", "1", "1", "-"],
                    ["Foguetes paraq.", "6", "4", "2", "-", "-"],
                    ["Fachos mao", "6", "4", "2", "-", "-"],
                    ["Sinais fumig.", "2", "2", "1", "-", "-"],
                    ["Espelho sinalizacao", "1", "1", "1", "-", "-"],
                    ["Jangada/Balsa", "Sim", "Sim", "-", "-", "-"],
                    ["Agulha magnetica", "Sim", "Sim", "-", "-", "-"],
                    ["Luzes navegacao", "Sim", "Sim", "Sim", "Sim", "Sim"],
                    ["Apito/Buzina", "Sim", "Sim", "Sim", "Sim", "Sim"],
                    ["Bussola", "-", "-", "Sim", "Sim", "-"],
                    ["Lanterna estanque", "Sim", "Sim", "Sim", "-", "-"],
                    ["1.os socorros", "Sim", "Sim", "Sim", "-", "-"],
                  ].map((row, i) => (
