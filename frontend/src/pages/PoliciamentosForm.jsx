...
import { generatePoliciamentosPDF } from "@/lib/pdfGenerator";
...
const ATIVIDADES_POLICIAMENTOS = [
  "POLICIAMENTO AO ABASTECIMENTO",
  "POLICIAMENTO A EVENTO",
  "POLICIAMENTO A OBRAS",
  "POLICIAMENTO A MANIFESTACAO",
...
export default function PoliciamentosForm() {
  ...
  const [form, setForm] = useState({
    ...
    despacho: "",
    numero_controlo: "",
    atividade: "POLICIAMENTO AO ABASTECIMENTO",
    navio: "",
    deslocacao_km: 0,
    ...
    // Extras
    pericias: 0, agravamento: 0,
    // Empenhamento
    bote: 0, lancha: 0, moto_agua: 0, viatura_4x4: 0, moto_4: 0, deslocacao: 0,
    agentes: [emptyAgente()],
    responsavel: "",
  });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      generatePoliciamentosPDF(result);
  ...
  return (
    <div data-testid="policiamentos-form-page" className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <Select value={form.atividade} onValueChange={v => updateField("atividade", v)}>
              ...
              <SelectContent>
                {ATIVIDADES_POLICIAMENTOS.map(a => <SelectItem key={a} value={a}>{a}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div>
      ...
      <div className="form-section">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div>
            <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Pericias</Label>
            <Input type="number" value={form.pericias} onChange={e => updateField("pericias", parseInt(e.target.value) || 0)} className="rounded-sm mt-1.5" data-testid="campo-pericias" />
          </div>
          <div>
            <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Agravamento</Label>
            <Input type="number" value={form.agravamento} onChange={e => updateField("agravamento", parseFloat(e.target.value) || 0)} className="rounded-sm mt-1.5" data-testid="campo-agravamento" />
          </div>
        </div>
      </div>
          ...
          {form.agentes.map((ag, idx) => (
            <Card key={idx} className="rounded-sm border-slate-200">
              <CardContent className="p-4">
                ...
                {/* Requisitado section for this agent */}
                <div className="mt-3">
                ...
                {/* Imposto section for this agent */}
                <div className="mt-3">
      ...
      <div className="flex items-center justify-between py-4 border-t border-slate-200">
        ...
        <Button
          onClick={handleSave}
          disabled={saving}
          className="rounded-sm text-xs font-semibold uppercase tracking-wide"
