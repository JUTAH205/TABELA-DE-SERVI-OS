...
import { Search, Plus, Pencil, Trash2, FileDown, Ship, Shield } from "lucide-react";
...
export default function ServicosTable() {
  ...
  }, [tipoFilter, comandoFilter, search]);
  ...
  const handleDownloadPDF = (servico) => {
    if (servico.tipo_formulario === "navios") {
      generateNaviosPDF(servico);
    } else {
      generatePoliciamentosPDF(servico);
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {servicos.length} servico(s) registado(s)
          </p>
        </div>
        <div className="flex gap-2">
          <Button
            onClick={() => navigate("/navios/novo")}
            ...
            data-testid="new-navios-btn"
          >
            <Ship className="h-4 w-4 mr-1.5" strokeWidth={1.5} />
            Novo Navios
          </Button>
          <Button
            onClick={() => navigate("/policiamentos/novo")}
            className="rounded-sm text-xs font-semibold uppercase tracking-wide"
            style={{ backgroundColor: "#DA291C", color: "white" }}
            data-testid="new-policiamentos-btn"
          >
        ...
        <CardContent className="p-4">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Select value={tipoFilter} onValueChange={setTipoFilter}>
              <SelectTrigger className="w-full sm:w-44 rounded-sm" data-testid="filter-tipo">
            ...
            <Select value={comandoFilter} onValueChange={setComandoFilter}>
              <SelectTrigger className="w-full sm:w-44 rounded-sm" data-testid="filter-comando">
                ...
                {loading ? (
                  ...
                  servicos.map((s) => (
                    <tr key={s.id} data-testid={`service-row-${s.numero_servico}`}>
                      ...
                      <td>{s.data}</td>
                      <td className="max-w-[200px] truncate">{s.utente}</td>
                      <td className="text-slate-500 max-w-[150px] truncate">{s.atividade}</td>
                      <td className="text-slate-500">{s.navio || "-"}</td>
                      ...
                      <td>
                        <div className="flex items-center justify-end gap-1">
                          <button
                            ...
                            title="Editar"
                            data-testid={`edit-btn-${s.numero_servico}`}
                          >
                            <Pencil className="h-3.5 w-3.5 text-slate-500" strokeWidth={1.5} />
                          </button>
                          <button
                            onClick={() => handleDownloadPDF(s)}
                            className="p-1.5 rounded-sm hover:bg-blue-50 transition-colors"
                            title="Descarregar PDF"
                            data-testid={`pdf-btn-${s.numero_servico}`}
                          >
                          ...
                          <button
                            ...
                            title="Eliminar"
                            data-testid={`delete-btn-${s.numero_servico}`}
                          >
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tem a certeza que pretende eliminar este servico? Esta acao nao pode ser revertida.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
